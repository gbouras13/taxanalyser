...
def create_depletion_report(sample, pre_depletion_fastq, post_lambda_fastq, post_chm13_fastq,post_filtlong_fastq, summary_file_path):
    ...
    summary = f"""Summary stats for {sample}:
        ...
        Proportion of reads mapped to host: {'N/A' if count_post_lambda_depletion == 0 else f'{round((count_post_lambda_depletion - count_post_chm13_depletion)/count_post_lambda_depletion*100, 2)}%'}
        Number of reads mapped to host (and depleted): {count_post_lambda_depletion - count_post_chm13_depletion}
